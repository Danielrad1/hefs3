...
interface AIHintsModelSelectionScreenProps {
  route: {
    params: {
      deckId: string;
      deckName: string;
...
export default function AIHintsModelSelectionScreen({ route, navigation }: AIHintsModelSelectionScreenProps) {
  const theme = useTheme();
  const { isPremiumEffective, usage, subscribe, fetchUsage } = usePremium();
  const { deckId, deckName, totalCards, items } = route.params;
  ...
  const modelOptions: ModelOption[] = [
    ...
    },
    {
      ...
      description: 'Premium quality hints with deeper insights',
      icon: 'sparkles',
      color: '#8B5CF6',
      features: [
        'Highest quality',
  ...
  const getUsageInfo = (tier: ModelTier) => {
    ...
    if (tier === 'basic') {
      return {
        used: usage.basicHintGenerations || 0,
        limit: usage.limits.basicHints || 3,
        canUse: (usage.basicHintGenerations || 0) < (usage.limits.basicHints || 3),
      };
    } else {
      return {
        used: usage.advancedHintGenerations || 0,
        limit: usage.limits.advancedHints || 1,
        canUse: (usage.advancedHintGenerations || 0) < (usage.limits.advancedHints || 1),
      };
    }
  };
  ...
  const handleContinue = () => {
    ...
    if (!usageInfo.canUse && !isPremiumEffective) {
      Alert.alert(
        'Limit Reached',
        `You've used all ${usageInfo.limit} ${selectedTier} hint generations this month. Upgrade to Premium for unlimited access.`,
        [
    ...
    logger.info('[AIHintsModelSelection] Selected model tier:', selectedTier);
              ...
              >
                ...
                <View style={[styles.timeEstimate, { backgroundColor: theme.colors.surface2 }]}>
                  ...
                  <Text style={[styles.timeText, { color: theme.colors.textSecondary }]}>
                    {option.estimatedTime} for {totalCards} cards
                  </Text>
                </View>
        ...
        {!isPremiumEffective && (
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.upgradeSubtitle}>Unlimited hints with both models</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  modelModel: { fontSize: 14, fontWeight: '500', marginTop: 2 },
  timeEstimate: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: s.sm,
